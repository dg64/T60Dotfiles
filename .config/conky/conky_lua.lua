--[[
  ...
  Acknowlegdments... -------------------------------
    ...
    --- calendar buttons ---
    (loosely) based on idea/code from mrPeachy (s11buttons)
    http://crunchbang.org/forums/viewtopic.php?pid=198010
  ...
  History: -----------------------------------------
    ...
    - 12/01/16 - mattz - V1.1 added stocks ticker, misc cleanup
...
function clickfunction()
  ...
  if start1 == 1 then
    ...
    local f1 = io.popen("ps -ef | grep click_conky | grep -v grep")
      local xproc = f1:read("*a")
    f1:close()
    if string.len(xproc) ~= 0 then
      os.execute("ps -ef | grep click_conky | grep -v grep | awk  '{print $2}' | xargs kill")
      -- io.write(xproc)
    ...
    os.execute("/usr/bin/xdotool search --name click_conky behave %@ mouse-enter getmouselocation >> /tmp/xdo &")

    -----  get "absolute 0" reference point for this conky window -----
    local f2 = io.popen("xwininfo -name 'click_conky' | grep 'Absolute'")
